...
def annotation_func(df, 
    ...
    for j in range(len(df[column_name])):
        ...
        if (j % 100 == 0 and j != 0):
            ...
            pickle.dump(annotated_df, open(f'retrieved_annotated_{column_name}_df2_{batch}.p', 'wb'))
...
def parralelism_metamap(numbers_of_cores,
    ...
    retrieved_path = [path for path in Path('./').iterdir() if path.stem == f'retrieved_annotated_{column_name}_df2']
    ...
    if update == True:
        ...
        df_processed = pickle.load(open(f'retrieved_annotated_{column_name}_df2.p', 'rb'))
    ...
    if par_core > 1:
        ...
        for i in range(par_core):
            if i == 0:
                current_data = (df[:round(len(df)/par_core)], i+1, batch_size, mm, column_name, unique_id, composite_phrase, filename, file_format, allow_acronym_variants, word_sense_disambiguation,
                ...
                current_data = (df[(round(len(df)/par_core))*i:(round(len(df)/par_core))*(i+1)], i+1, batch_size, mm, column_name, unique_id, composite_phrase, filename, file_format, allow_acronym_variants, word_sense_disambiguation,
                        allow_large_n, strict_model, relaxed_model, allow_overmatches, allow_concept_gaps, term_processing, no_derivational_variants,
                        derivational_variants, ignore_word_order, unique_acronym_variants, prefer_multiple_concepts, ignore_stop_phrases, compute_all_mappings,
                        prune, mm_data_version, verbose, exclude_sources, restrict_to_sources, restrict_to_sts, exclude_sts, no_nums)
                data.append(current_data)
            else:
                current_data = (df[round(len(df)/par_core)*i:], i+1, batch_size, mm, column_name, unique_id, composite_phrase, filename, file_format, allow_acronym_variants, word_sense_disambiguation,
                        allow_large_n, strict_model, relaxed_model, allow_overmatches, allow_concept_gaps, term_processing, no_derivational_variants,
                        derivational_variants, ignore_word_order, unique_acronym_variants, prefer_multiple_concepts, ignore_stop_phrases, compute_all_mappings,
                        prune, mm_data_version, verbose, exclude_sources, restrict_to_sources, restrict_to_sts, exclude_sts, no_nums)
                data.append(current_data)
    else:
        data = [(df, 1, batch_size, mm, column_name, unique_id, composite_phrase, filename, file_format, allow_acronym_variants, word_sense_disambiguation,
                        allow_large_n, strict_model, relaxed_model, allow_overmatches, allow_concept_gaps, term_processing, no_derivational_variants,
                        derivational_variants, ignore_word_order, unique_acronym_variants, prefer_multiple_concepts, ignore_stop_phrases, compute_all_mappings,
                        prune, mm_data_version, verbose, exclude_sources, restrict_to_sources, restrict_to_sts, exclude_sts, no_nums)]
    ...
    concat_df = pickle.load(open(f'retrieved_annotated_{column_name}_df2_1.p', 'rb'))
    if numbers_of_cores > 1:
        for i in range(par_core):
            df_dynamic = pickle.load(open(f'retrieved_annotated_{column_name}_df2_{i+1}.p', 'rb'))
            concat_df = pd.concat([concat_df, df_dynamic])


    pickle.dump(concat_df, open(f'retrieved_annotated_{column_name}_df.p', 'wb'))
    ...
    pickle.dump(final_df, open(f'retrieved_annotated_{column_name}_df2.p', 'wb'))
